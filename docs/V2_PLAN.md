...
> **Обновление:** блок D (авто-перевыдача роли «Рофлер», `role_service.py`) с тех пор
> **удалён** — бот больше не выдаёт и не снимает роли сам, только публикует топ 3+2
> в embed. Все ссылки на `role_service.py`, `ROLE_ROFLER_ID`, `ROLE_REASSIGN_ENABLED`
> и связанные тесты ниже — историческая запись, актуальному коду не соответствуют.
