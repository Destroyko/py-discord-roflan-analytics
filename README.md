...
| Команда | Описание |
|---------|----------|
| `/show_leaderboard` | TOP по дурке и рофлинкам за месяц из SQLite (без скана, как в `LEADERBOARD_CHANNEL_ID`); ответ публичный. Будущий месяц или период раньше первых данных бота — шутливая отписка вместо TOP; совсем некорректный месяц (не 1–12) — команда молча ничего не отвечает |
